...
The TPC is plotted in Supp. Figure 1.
...
We used these values of total proportion mortality, along with the total duration of each stage in that study (given Supp. Table 1), to calculate the daily probability of survival:
...
This assumes that the internal wood temperature does not exceed the maximum air temperature, or get lower than the temperature of the soil 1 m below the surface. The relative weight of $T_{max}$ and $T_{soil}$ is given by $p_t$, which can take a value between 0 and 1. $p_t$ is a function of the relative humidity, $H$. Specifically, we assumed a linear relationship with humidity, and $p_t$ was calculated as the *inverse logit(?)* of:
...
p <- ilogit(int + beta*rh) # Take the inverse logit to go from log-odds to probability
