...
# merge_temp <- na.omit(merge_temp)
    # There's a gap in sflow data DOY 180 - 220
    # If we omit the NAs though, greta model doesn't seem to make predictions (so there's just a big gap)

treeTemps <- as_data(na.omit(merge_temp)$mean_d) # We DO need to omit NAs for this though
...
distribution(treeTemps) <- normal(mean_temp, sd)
  # ^THIS now doesn't work since treeTemps (omit NAs in sflow) and mean_temp (full dataset) are different lengths
...
## THESE plots also don't work now with treeTemps vs merge_temp diff lengths
...
ggplot()+
  geom_point(data = merge_temp, aes(x=DOY, y=mean_d), col='brown')+
  geom_point(data = merge_temp, aes(x=DOY, y=mean_u), col='orange')+
  geom_point(data = lm_pred, aes(x=DOY, y=lm_pred), col="red")+
  geom_point(data = plot_data, aes(x=DOY, y=pred_temp), col="blue")
